...
public class AStarPathFinder extends PathFinderBase {
        ...
        while ((maxDepth < maxSearchDistance) && !open.isEmpty()) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                if (mapManager.isWithinBoard(newCoord)) {
                    if (directionToInt(neighbourDirection[i]) == -1) continue;
                    Node newNode = nodes[neighbourX[i]][neighbourY[i]][directionToInt(neighbourDirection[i])];

                    if (!mapManager.isWall(neighbourX[i], neighbourY[i])) {
                        // valid neighbour
                        if (!closed.contains(newNode)) {
                            // relax : update neighbours
    ...
    /**
     * Gets the penalty for each cell. used for calculating costs.
     * @param x
     * @param y
     * @return
     */
    private float tilePenalty(int x, int y) {
        ...
        float tilecost = 1.0f;
        // basis
        if (cell.type == CellType.ROAD && mapManager.hasSeenTile(x, y)) {
            tilecost = 5;
        } else if (cell.type == CellType.ROAD && !mapManager.hasSeenTile(x, y)) {
            // unseen tile is half the Lava
            tilecost = 25;
